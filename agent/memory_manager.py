...
_FENCE_CLOSE_TAG_RE = re.compile(
    r'(^|\n)[ \t]*</\s*memory-context\s*>[ \t]*(?=\n|$)',
    re.IGNORECASE,
)
...
def sanitize_context(text: str) -> str:
    ...
    # Preserve opening tags until after the greedy fallback.  If we strip the
    # open fence first, an unterminated memory-context block loses the marker
    # that lets us remove the leaked recalled payload.
