...
class GatewayStreamConsumer:
    ...
    def _reset_segment_state(self, *, preserve_no_edit: bool = False) -> None:
        if preserve_no_edit and self._message_id == "__no_edit__":
            return
        # Retain the finalized visible text before any reset/reuse, so
        # has_delivered_text remains correct across segment boundaries.
        ...
        # Reuse the existing visible bubble when the local merge feature is
        # enabled, but only after recording its finalized text above.
    ...
    def _clean_for_display(text: str) -> str:
        ...
        user.
        """
        # Local patch: also strip leaked memory-context fences before the
        # upstream media-directive stripper runs, so internal context blocks
        # do not flash through onto the chat display.
        try:
            from agent.memory_manager import sanitize_context
            cleaned = sanitize_context(text)
        except Exception:
            cleaned = text
        return _BasePlatformAdapter.strip_media_directives_for_display(cleaned)
